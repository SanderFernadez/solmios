...
- [ ] 3.1 `make:module Compras`
- [ ] 3.2 Modelos: `PurchaseRequisitions`(+`RequisitionItems`) · `PurchaseOrders`(+`PurchaseOrderItems`) · `GoodsReceipts`(+`ReceiptItems`)
- [ ] 3.3 Wiring + permisos módulo `purchasing`. Proveedores por connector a `treasury` (NO importar)
- [ ] 3.4 GATE: `arckode analyze` ✅ · RUN_MIGRATE local

### COM-1 — Requisiciones  ⟵ COM-0
- [ ] 4.1 Crear requisición con líneas (itemId opcional + descripción libre, cantidad, notas). Estado `draft`
- [ ] 4.2 Ciclo: `draft → submitted → approved | rejected` (aprobación con permiso `purchasing:edit`)
- [ ] 4.3 Número correlativo `REQ-YYYY-NNNN` (contador atómico en configuration)
- [ ] 4.4 Tests + QA

### COM-2 — Órdenes de compra  ⟵ COM-1
- [ ] 5.1 OC desde requisición aprobada (o directa): supplierId (valida vía connector treasury), líneas con precio unitario, subtotal/impuesto/total
- [ ] 5.2 Ciclo: `draft → sent → received | closed | cancelled`. Número `OC-YYYY-NNNN`
- [ ] 5.3 Impuesto desde configuration (NO hardcodear); moneda del hotel
- [ ] 5.4 Tests + QA

### COM-3 — Recepción → suma stock  ⟵ COM-2 + INV-2
- [ ] 6.1 Goods receipt (total o parcial) contra una OC: líneas recibidas (cantidad ≤ pendiente)
- [ ] 6.2 Connector `compras→inventario`: por cada línea recibida con itemId → `applyMovement(in, qty, unitCost)` (dedup por receiptItemId)
- [ ] 6.3 OC pasa a `received` (o parcial) según lo recibido
- [ ] 6.4 Tests (recepción total/parcial, suma stock, costo promedio, dedup) + QA

### COM-4 — OC facturada → genera Gasto  ⟵ COM-3
- [ ] 7.1 Marcar OC facturada (invoiceNumber del proveedor) → connector `compras→gastos`: crea `Expense` (`source='purchase_order'`, `sourceId=poId`, `supplierId`, amount=total, category) — dedup por source+sourceId
- [ ] 7.2 El Gasto ya pega en caja + contabilidad (connectors existentes). Verificar cuenta de gasto correcta (`expenseAccountForSource` → agregar `purchase_order`)
- [ ] 7.3 Tests (genera 1 gasto, no duplica, monto correcto) + QA

## INT — Integración con el POS

### INT-1 — Recetas → venta descuenta stock  ⟵ INV-2
- [ ] 8.1 Modelo `MenuItemRecipes` (menuItemId, inventoryItemId, quantity) — BOM por ítem de menú. En `inventario` o `restaurant`? → en `inventario` (dueño del stock), el restaurant no lo importa
- [ ] 8.2 UI en la carta (restaurante): asignar receta a un ítem (opcional)
- [ ] 8.3 Connector `restaurant→inventario`: al cobrar/servir una comanda, por cada línea con receta → `applyMovement(out, qty*recipeQty)` (dedup por orderLineId). Best-effort (no rompe la venta)
- [ ] 8.4 Tests (venta descuenta, sin receta no descuenta, dedup) + QA
