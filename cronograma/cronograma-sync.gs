/**
 * CRONOGRAMA MANAGER HOTEL -> GITLAB SYNC
 * =========================================
...
function pushToGitLab() {
  try {
    ...
    for (var i = 0; i < tasks.length; i++) {
      ...
      var desc = "**Modulo:** " + t.module + "\n"
               + "**Submodulo:** " + t.submodule + "\n"
               + "**ID Plan:** " + t.id + "\n"
      ...
      if (t.url) {
        ...
        setCell(sheet, t.row, 10, url);
        setCell(sheet, t.row, 1, t.id);
        count.created++;
      }
    }
    ...
    for (var i = 0; i < issues.length; i++) {
      ...
      for (var j = 0; j < tasks.length; j++) {
        if (tasks[j].url === gl.web_url) {
          ...
          setCell(sheet, tasks[j].row, 8, glStatus);
          matched = true;
          count++;
          break;
        }
...
function showHelp() {
  var html = HtmlService.createHtmlOutput(
    ...
    + "I: Sprint<br>"
    + "J: URL GitLab Issue (se llena automaticamente al hacer push)</p>"
    + "<p><b>Labels GitLab:</b> Modulo::X, Sprint::X, Prioridad::X, Estado::X, Responsable::X</p>"
    + "<p><b>Tokens:</b> GitLab > Settings > Access Tokens > scope 'api'</p>"
    + "<p><b>Proyecto:</b> underworf1/solmios</p>"
    + "<hr><p style='color:#666'>v1.0 — Manager Hotel</p>"
