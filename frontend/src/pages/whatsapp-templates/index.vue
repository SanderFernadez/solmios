<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      ...
      <button @click="openNew" class="bg-cyan text-navy font-extrabold text-sm px-5 py-2.5 rounded-xl hover:shadow-lg cursor-pointer">+ Nueva Plantilla</button>
    </div>
    ...
    <div v-else-if="templates.length === 0" class="card p-12 text-center">
      ...
      <button @click="openNew" class="px-5 py-2.5 bg-cyan text-navy rounded-xl text-sm font-bold cursor-pointer">+ Crear plantilla</button>
    </div>
      ...
      <div v-for="t in templates" :key="t.id" class="card p-5 cursor-pointer hover:shadow-lg transition-all flex flex-col" @click="openEdit(t)">
        <div class="flex items-start justify-between mb-2">
          <div class="flex items-center gap-2">
            <span class="text-lg">{{ categoryIcon(t.category) }}</span>
      ...
      <div v-if="modal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="modal.show=false">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div class="p-5 space-y-4">
            <div class="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select v-model="form.category" class="w-full px-4 py-2.5 rounded-xl border border-border text-sm cursor-pointer">
                  ...
                  <option value="checkin">🛎️ Check-in</option>
            ...
            <!-- Preview -->
            <div v-if="form.body" class="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
              <div class="text-[10px] font-bold text-emerald-700 uppercase mb-2">📱 Vista previa</div>
              <p class="text-xs text-gray-800 whitespace-pre-wrap">{{ preview }}</p>
            </div>
          </div>
          <div class="p-5 border-t border-border bg-surface/50 flex gap-3 justify-end sticky bottom-0">
            <button v-if="modal.edit" @click="deleteTemplate" class="px-5 py-2.5 border border-coral/30 text-coral rounded-xl text-sm font-bold cursor-pointer mr-auto">🗑️ Eliminar</button>
...
function categoryIcon(c?: string) {
  const icons: Record<string, string> = { general: '💬', reservation: '📅', checkin: '🛎️', checkout: '🚪', payment: '💳', marketing: '📣' }
  return icons[c || 'general'] || '💬'
}
