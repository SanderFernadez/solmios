<template>
  <div>
    ...
    <SectionCard title="Suscripciones por hotel" :subtitle="`${subscriptions.length} hoteles · MRR total $${mrrTotal.toLocaleString()}/mes`">
      <template #actions>
        <div class="relative">
          <input v-model="searchQuery" type="text" placeholder="Buscar hotel..." class="w-64 h-9 pl-9 pr-4 rounded-lg border border-white/20 text-sm bg-white/10 text-white placeholder-white/50 focus:outline-none focus:border-cyan">
...
}[s] ?? 'bg-danger/10 text-danger') // expired / canceled
