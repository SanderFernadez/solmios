<template>
  <div class="p-6 max-w-3xl mx-auto space-y-6">
    ...
    <SectionCard title="Catálogo de módulos" body-class="p-0">
      <template #actions>
        <button @click="save" :disabled="saving || !dirty"
          class="px-5 py-2.5 bg-cyan text-navy rounded-xl text-sm font-extrabold hover:shadow-lg transition-all disabled:opacity-40 cursor-pointer shrink-0">
          {{ saving ? 'Guardando…' : 'Guardar cambios' }}
        </button>
      </template>

      <div class="divide-y divide-border">
        <div v-for="m in catalog" :key="m.key">
          <!-- Módulo -->
