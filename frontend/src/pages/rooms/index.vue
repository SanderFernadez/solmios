  ...
  <div>
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex gap-2">
        ...
        <button @click="openNew" class="bg-cyan text-navy font-extrabold text-sm px-5 py-2.5 rounded-xl hover:shadow-lg transition cursor-pointer">
          + Nueva
        </button>
        <button @click="openBatch" class="bg-surface border border-border text-navy font-bold text-sm px-5 py-2.5 rounded-xl hover:shadow transition cursor-pointer">
          + Crear en Lote
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-6 gap-3 mb-6">
      <div v-for="s in stats" :key="s.label" class="bg-white rounded-xl border border-border p-3 text-center">
    ...
    <div v-if="rooms.length === 0 && !loading" class="text-center py-12">
      ...
      <p class="text-text-muted mt-2">No hay habitaciones. Creá la primera con "+ Nueva" o "+ Crear en Lote".</p>
    </div>

    <!-- Rooms by Type -->
    <div v-for="rt in paginatedRoomTypes" :key="rt.type" class="mb-6">
      <div class="flex items-center gap-3 mb-3 px-1">
        <span class="text-base">{{ rt.icon }}</span>
        <h3 class="text-sm font-black text-navy">{{ rt.type }}</h3>
        <span class="text-[10px] text-text-muted">({{ rt.rooms.length }})</span>
        <span class="text-[10px] font-bold text-teal">{{ rt.available }} disponibles</span>
        <span class="text-[10px] font-bold text-coral">{{ rt.occupied }} ocupadas</span>
        <span v-if="rt.cleaning" class="text-[10px] font-bold text-amber-500">{{ rt.cleaning }} limpieza</span>
      </div>
      <div class="bg-white rounded-2xl border border-border overflow-hidden">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 p-3">
          <div v-for="room in rt.rooms" :key="room.id" @click="openDetail(room)"
            class="rounded-xl border-2 p-4 cursor-pointer hover:shadow-lg transition-all relative"
            :class="roomCardClass(room.status)">
            <div class="absolute top-2 right-2 flex items-center gap-1.5">
              <span class="w-2.5 h-2.5 rounded-full border border-white" :class="statusDot(room.status)"></span>
              <span class="text-[9px] font-bold uppercase text-text-muted">{{ statusLabel(room.status) }}</span>
            </div>
            <div class="mb-3 pt-2">
            ...
            <div class="flex items-baseline gap-1 mb-2">
              ...
              <span class="text-[10px] text-text-muted font-medium">/noche</span>
            </div>
            <div class="flex items-center gap-2 text-[10px] text-text-muted mb-2">
              <span class="flex items-center gap-0.5">👤 {{ room.maxGuests }}p</span>
              <span>·</span>
              <span>🏢 Piso {{ room.floor }}</span>
              <span v-if="room.surfaceArea">·</span>
              <span v-if="room.surfaceArea">{{ room.surfaceArea }}m²</span>
            </div>
            <div class="flex flex-wrap gap-1 mt-2">
              <span v-for="a in (room.amenities||[]).slice(0, 4)" :key="a"
                class="px-1.5 py-0.5 border border-border rounded text-[8px] text-text-secondary font-medium capitalize">
                {{ amenityLabel(a) }}
              </span>
              <span v-if="(room.amenities||[]).length > 4"
                class="px-1.5 py-0.5 bg-surface rounded text-[8px] text-text-muted">
    ...
    <div v-if="totalPages > 1" class="flex items-center justify-between mt-4 px-2">
      ...
      <div class="flex items-center gap-1">
        <button @click="page = 1" :disabled="page <= 1"
          class="w-8 h-8 rounded-lg border border-border flex items-center justify-center text-xs font-bold cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed hover:bg-surface">«</button>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div class="p-5 border-b border-border flex items-center justify-between sticky top-0 bg-white z-10">
            <div class="flex items-center gap-3">
              ...
              <span class="w-3 h-3 rounded-full border border-white" :class="statusDot(detailRoom?.status||'')"></span>
              <span class="text-xs font-bold text-text-muted uppercase">{{ statusLabel(detailRoom?.status||'') }}</span>
            </div>
            <button @click="detailModal.show=false" class="w-8 h-8 rounded-lg bg-surface flex items-center justify-center hover:bg-gray-200 cursor-pointer">✕</button>
          </div>
          ...
          <div class="p-5 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="bg-surface rounded-xl p-3">
                <div class="text-[9px] text-text-muted uppercase font-bold">Tipo</div>
                <div class="text-sm font-black text-navy">{{ typeLabel(detailRoom?.type||'') }}</div>
              </div>
              <div class="bg-surface rounded-xl p-3">
                <div class="text-[9px] text-text-muted uppercase font-bold">Precio</div>
                <div class="text-sm font-black text-navy">${{ detailRoom?.basePrice }} <span class="text-[10px] font-medium text-text-muted">/noche</span></div>
              </div>
              <div class="bg-surface rounded-xl p-3">
                <div class="text-[9px] text-text-muted uppercase font-bold">Piso</div>
                <div class="text-sm font-bold text-navy">{{ detailRoom?.floor }}</div>
              </div>
              <div class="bg-surface rounded-xl p-3">
                <div class="text-[9px] text-text-muted uppercase font-bold">Capacidad</div>
                <div class="text-sm font-bold text-navy">{{ detailRoom?.maxGuests }} personas</div>
              </div>
              <div class="bg-surface rounded-xl p-3">
                <div class="text-[9px] text-text-muted uppercase font-bold">Baños</div>
                <div class="text-sm font-bold text-navy">{{ detailRoom?.bathrooms }}</div>
              </div>
              <div class="bg-surface rounded-xl p-3">
                <div class="text-[9px] text-text-muted uppercase font-bold">Superficie</div>
            ...
            <div v-if="detailRoom?.status === 'occupied' && detailRoom?.guestName" class="bg-coral/5 border border-coral/20 rounded-xl p-4">
              <div class="text-[9px] text-coral uppercase font-bold mb-1">Huésped en Casa</div>
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-1.5">
                <span v-for="a in (detailRoom?.amenities||[])" :key="a"
                  class="px-2 py-1 bg-surface rounded-lg border border-border text-[10px] text-navy font-medium">
                  {{ amenityIcon(a) }} {{ amenityLabel(a) }}
                </span>
                <span v-if="!detailRoom?.amenities?.length" class="text-[10px] text-text-muted">Sin amenities configurados</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-2">
                <button v-for="opt in statusOptions" :key="opt.value"
                  ...
                  class="p-2.5 rounded-xl border-2 text-left transition cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed hover:shadow-sm"
                  :class="detailRoom?.status === opt.value ? 'border-navy bg-navy/5' : 'border-border'">
                  <div class="flex items-center gap-2">
                    <span class="w-2.5 h-2.5 rounded-full" :class="statusDot(opt.value)"></span>
                    <div>
                      <div class="text-xs font-extrabold text-navy">{{ opt.label }}</div>
                      <div class="text-[8px] text-text-muted leading-tight">{{ opt.desc }}</div>
                    </div>
                  </div>
                </button>
              </div>
          ...
          <div class="p-5 border-t border-border bg-surface/50 flex gap-3 justify-between sticky bottom-0">
            <button @click="deleteRoomFromDetail"
              class="px-4 py-2.5 border border-coral/30 text-coral rounded-xl text-sm font-bold cursor-pointer hover:bg-coral/5">🗑️ Eliminar</button>
            <div class="flex gap-2">
              ...
              <button @click="openEditFromDetail" class="px-4 py-2.5 bg-navy text-white rounded-xl text-sm font-bold cursor-pointer hover:bg-navy/90">✏️ Editar</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
          <div class="p-5 border-b border-border flex items-center justify-between sticky top-0 bg-white z-10">
            <h3 class="text-lg font-black text-navy flex items-center gap-2">
              <span>📦</span> Crear Habitaciones en Lote
            </h3>
            <button @click="batchModal.show=false" class="w-8 h-8 rounded-lg bg-surface flex items-center justify-center hover:bg-gray-200 cursor-pointer">✕</button>
          </div>
          ...
          <div class="p-5 space-y-5">
            <div>
              ...
              <div class="grid grid-cols-2 gap-2">
                <button v-for="opt in typeOptions" :key="opt.value"
                  ...
                  class="p-3 rounded-xl border-2 text-left transition cursor-pointer"
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div class="text-[9px] text-text-muted">{{ opt.desc }}</div>
                    </div>
                  </div>
                </button>
              </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="block text-[9px] text-text-muted font-bold mb-1">Desde N°</label>
                ...
                <div>
                  <label class="block text-[9px] text-text-muted font-bold mb-1">Hasta N°</label>
              ...
              <div v-if="batchCount > 0 && batchCount <= 100" class="mt-3 bg-emerald-50 border border-emerald-200 rounded-xl p-3">
                <div class="flex items-center gap-2">
                  <span class="text-sm">📋</span>
                  <span class="text-xs font-bold text-emerald-800">{{ batchCount }} habitaciones</span>
                  <span class="text-[10px] text-emerald-600">{{ batchPreview }}</span>
                </div>
              </div>
              <div v-else-if="batchCount > 100" class="mt-3 bg-red-50 border border-red-200 rounded-xl p-3">
                <span class="text-xs font-bold text-red-600">Máximo 100 por lote</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-3 gap-3">
                <div><label class="block text-[9px] text-text-muted font-bold mb-1">Precio Base $</label><input v-model.number="batchForm.basePrice" type="number" min="0" class="w-full px-3 py-2 rounded-xl border border-border text-sm font-bold text-navy" /></div>
                <div><label class="block text-[9px] text-text-muted font-bold mb-1">Capacidad</label><input v-model.number="batchForm.capacity" type="number" min="1" max="20" class="w-full px-3 py-2 rounded-xl border border-border text-sm" /></div>
                <div><label class="block text-[9px] text-text-muted font-bold mb-1">Piso</label><input v-model.number="batchForm.floor" type="number" min="0" class="w-full px-3 py-2 rounded-xl border border-border text-sm" /></div>
                <div><label class="block text-[9px] text-text-muted font-bold mb-1">Baños</label><input v-model.number="batchForm.bathrooms" type="number" min="0" class="w-full px-3 py-2 rounded-xl border border-border text-sm" /></div>
                <div><label class="block text-[9px] text-text-muted font-bold mb-1">Superficie m²</label><input v-model.number="batchForm.surfaceArea" type="number" min="0" class="w-full px-3 py-2 rounded-xl border border-border text-sm" /></div>
                <div class="flex items-end"><label class="flex items-center gap-2 cursor-pointer bg-surface rounded-xl px-3 py-2 w-full"><input v-model="batchForm.onlineBooking" type="checkbox" class="w-4 h-4 rounded text-cyan" /><span class="text-[10px] font-bold text-navy">Venta Online</span></label></div>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-1.5 bg-surface rounded-xl p-3">
                <label v-for="a in batchAmenities" :key="a.key" class="flex items-center gap-2 cursor-pointer hover:bg-white rounded-lg px-2 py-1.5 transition">
                  ...
                  <span class="text-xs text-navy">{{ a.icon }} {{ a.label }}</span>
                </label>
              </div>
            </div>
          </div>
      ...
      <div v-if="modal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="modal.show=false">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          <div class="p-5 border-b border-border flex items-center justify-between sticky top-0 bg-white z-10">
            ...
            <button @click="modal.show=false" class="w-8 h-8 rounded-lg bg-surface flex items-center justify-center hover:bg-gray-200 cursor-pointer">✕</button>
          </div>
...
const typeOptions = [
  { value: 'single', label: 'Individual', desc: '1 cama · 1 persona', icon: '🛏️' },
  { value: 'double', label: 'Doble', desc: '1 cama grande · 2 personas', icon: '🛌' },
  { value: 'twin', label: 'Twin', desc: '2 camas · 2 personas', icon: '🛏️🛏️' },
  { value: 'suite', label: 'Suite', desc: 'Sala + hab · 2-4 personas', icon: '👑' },
  { value: 'deluxe', label: 'Deluxe', desc: 'Vista premium · 2 personas', icon: '✨' },
  { value: 'presidential', label: 'Presidencial', desc: 'Máximo lujo · 4 personas', icon: '💎' },
  { value: 'family', label: 'Familiar', desc: '4+ personas · niños', icon: '👨‍👩‍👧‍👦' },
]

const amenityOptions = [
  { key:'wifi', label:'WiFi', icon:'📡' },{ key:'tv', label:'TV', icon:'📺' },{ key:'ac', label:'Aire Acond.', icon:'❄️' },
  { key:'heating', label:'Calefacción', icon:'🔥' },{ key:'safe', label:'Caja Fuerte', icon:'🔒' },{ key:'minibar', label:'Minibar', icon:'🧊' },
  { key:'kitchen', label:'Cocina', icon:'🍳' },{ key:'fridge', label:'Nevera', icon:'🧊' },{ key:'microwave', label:'Microondas', icon:'📡' },
  { key:'coffee_maker', label:'Cafetera', icon:'☕' },{ key:'washer', label:'Lavadora', icon:'🧺' },{ key:'dishwasher', label:'Lavavajillas', icon:'🍽️' },
  { key:'hair_dryer', label:'Secador', icon:'💨' },{ key:'iron', label:'Plancha', icon:'👕' },{ key:'balcony', label:'Balcón', icon:'🌅' },
  { key:'bathtub', label:'Bañera', icon:'🛁' },{ key:'work_desk', label:'Escritorio', icon:'🪑' },
]
...
function amenityIcon(key: string): string {
  const found = amenityOptions.find(a => a.key === key)
...
const TYPE_ICON: Record<string, string> = {
  single: '🛏️', double: '🛌', twin: '🛏️🛏️', triple: '🛏️🛏️🛏️', quad: '🛏️🛏️🛏️🛏️',
  suite: '👑', deluxe: '✨', presidential: '💎', family: '👨‍👩‍👧‍👦',
}
