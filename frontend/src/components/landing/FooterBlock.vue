      ...
      <div class="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
          ...
          <ul class="space-y-2 text-sm">
            <li v-if="hotel.address" class="text-white/80">📍 {{ fullAddress }}</li>
            <li v-if="hotel.phone" class="text-white/80">📞 {{ hotel.phone }}</li>
            <li v-if="hotel.email" class="text-white/80">✉️ {{ hotel.email }}</li>
            <li v-if="hotel.website">
              <a :href="hotel.website" target="_blank" rel="noopener noreferrer" class="text-cyan hover:text-cyan-light font-bold">
                🌐 Sitio web
              </a>
            </li>
          </ul>
