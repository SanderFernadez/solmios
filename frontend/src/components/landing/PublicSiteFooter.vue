<template>
  <!-- Footer del sitio público, compartido entre la landing (pages/landing/index.vue) y
       las páginas legales (pages/legal/*). Antes vivía inline en la landing con
       `legalLinks` como strings sueltos sin href (Privacidad/Términos/Cookies no eran
       clickeables y "Cookies" no tenía página) — acá quedan como router-link reales a
       los 3 documentos legales que sí existen. -->
  <footer class="bg-navy-deep pt-16 pb-8 px-6 border-t border-white/5">
    <div class="max-w-7xl mx-auto">
      <div class="grid md:grid-cols-12 gap-10 mb-12">
        ...
        <!-- Legal (columna propia: son 3 documentos reales, no un link suelto) -->
        <div class="md:col-span-2">
          <h4 class="text-sm font-bold text-white mb-4">Legal</h4>
          <div class="space-y-2.5">
            <router-link v-for="link in legalLinks" :key="link.to" :to="link.to"
              class="block text-sm text-white/40 hover:text-blue-400 hover:translate-x-1 transition-all duration-200">{{ link.label }}</router-link>
          </div>
        </div>
        <!-- Newsletter / CTA mini -->
        <div class="md:col-span-2">
      ...
      <div class="border-t border-white/8 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div class="flex gap-6">
          <router-link v-for="link in legalLinks" :key="link.to" :to="link.to" class="text-sm text-white/30 hover:text-blue-400 transition-colors">{{ link.shortLabel }}</router-link>
        </div>
      </div>
    </div>
  </footer>
...
const legalLinks = [
  { to: '/legal/privacidad', label: 'Política de Privacidad', shortLabel: 'Privacidad' },
  { to: '/legal/terminos', label: 'Términos y Condiciones', shortLabel: 'Términos' },
  { to: '/legal/eliminacion-datos', label: 'Eliminación de Datos', shortLabel: 'Eliminación de Datos' },
]
</script>
