...
// parametrizada por `service` para que recepción y restaurante compartan la MISMA UI apuntando
// a su propio punto de venta (el backend ya separa turno/movimientos por `register`). Antes esto
// vivía duplicado a mano en pages/caja/index.vue; ahora esa página es un wrapper de 3 líneas.
import { ref, computed, onMounted } from 'vue'
import type { CashMovement, CashShift, CashStats, Reconcile } from '@/services/Caja.service'
...
interface CashServiceLike {
  ...
  closeShift: (id: string, countedAmount: number, denominations?: string) => Promise<CashShift>
...
async function doCloseShift() {
  ...
  closing.value = true
  try {
    await props.service.closeShift(currentShift.value.id, countedAmount.value)
