<template>
  <div class="flex flex-col rounded-[20px] border border-white/8 bg-[#0B1526] p-5">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <h2 class="text-xs font-black uppercase tracking-wider text-white">Ingresos</h2>
        <div class="mt-2 flex items-baseline gap-2">
          <span class="text-4xl font-black tabular-nums tracking-tight text-white">${{ animatedToday }}</span>
          <span class="text-xs font-bold text-slate-400">Hoy</span>
        </div>
        <div v-if="trendPct !== null" class="mt-1.5 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-extrabold"
          :class="trendPct >= 0 ? 'bg-[#22C55E]/10 text-[#22C55E]' : 'bg-[#EF4444]/10 text-[#EF4444]'">
          {{ trendPct >= 0 ? '▲' : '▼' }} {{ trendPct >= 0 ? '+' : '' }}{{ trendPct }}% <span class="font-semibold text-slate-400">vs ayer</span>
        </div>
      </div>
      ...
      <div class="flex items-center gap-0.5 rounded-xl border border-white/10 bg-white/3 p-1">
        <button v-for="r in RANGES" :key="r.key" @click="range = r.key"
          ...
          :class="range === r.key ? 'bg-[#2563EB] text-white' : 'text-slate-400 hover:text-white'">
          {{ r.label }}
        </button>
      </div>
    </div>
    ...
    <div class="relative mt-4 flex-1" style="min-height: 180px">
      <svg v-if="points.length > 1" class="h-full w-full" viewBox="0 0 400 160" preserveAspectRatio="none">
        ...
        <!-- grid -->
        <line v-for="i in 4" :key="i" x1="0" :y1="(160 / 4) * i" x2="400" :y2="(160 / 4) * i" stroke="rgba(255,255,255,0.05)" stroke-width="1" />
        <polygon :points="`0,160 ${svgPoints} 400,160`" fill="url(#revGrad)" />
        <polyline :points="svgPoints" fill="none" stroke="#3B82F6" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"
          style="filter: drop-shadow(0 0 6px rgba(59,130,246,0.5))" />
      </svg>
      <div v-else class="grid h-full place-items-center text-xs text-slate-500">
        {{ loading ? 'Cargando ingresos…' : 'Sin datos de facturación en el período' }}
      </div>

      <!-- Y max label -->
      <span v-if="maxValue > 0" class="absolute right-0 top-0 text-[9px] font-bold tabular-nums text-slate-500">${{ compact(maxValue) }}</span>
    </div>

    <!-- X labels -->
    <div v-if="points.length > 1" class="mt-2 flex justify-between text-[9px] font-bold text-slate-500">
