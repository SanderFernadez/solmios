...
export class AttendanceController {
  ...
  async clockIn(req: HttpRequest) {
    ...
    const hotelId = (req as any).hotelId; const employeeId = b.employeeId ?? (req as any).userId
    return { status: 201, body: await this.service.clockIn(employeeId, hotelId, b.method) }
  }
  async clockOut(req: HttpRequest) {
    const b = req.body as Record<string, any>
    return { status: 200, body: await this.service.clockOut(b.employeeId ?? (req as any).userId, (req as any).hotelId) }
  }
  ...
  async manualRecord(req: HttpRequest) {
    ...
    return { status: 201, body: await this.service.manualRecord(b.employeeId, (req as any).hotelId, { clockIn: b.clockIn, clockOut: b.clockOut, notes: b.notes }, (req as any).userId) }
  }
  async getToday(req: HttpRequest) { return { status: 200, body: await this.service.getToday(req.params.employeeId) } }
  async listRecords(req: HttpRequest) { const q = req.query as any; q.hotelId = (req as any).hotelId ?? q.hotelId; return { status: 200, body: await this.service.listRecords(q) } }
  async getReport(req: HttpRequest) { const hotelId = (req as any).hotelId ?? (req.query as any).hotelId; const { from, to } = req.query as any; return { status: 200, body: await this.service.getReport(hotelId, from, to) } }
  ...
  async listSchedules(req: HttpRequest) { return { status: 200, body: await this.service.listSchedules((req as any).hotelId ?? (req.query as any).hotelId) } }
  async deleteSchedule(req: HttpRequest) { await this.service.deleteSchedule(req.params.id); return { status: 204, body: null } }

  async getConfig(req: HttpRequest) { const hotelId = (req as any).hotelId ?? (req.query as any).hotelId; if (!hotelId) return { status: 400, body: { error: 'hotelId requerido' } }; return { status: 200, body: await this.service.getConfig(hotelId) } }
  async updateConfig(req: HttpRequest) { const hotelId = (req as any).hotelId ?? (req.query as any).hotelId; if (!hotelId) return { status: 400, body: { error: 'hotelId requerido' } }; return { status: 200, body: await this.service.updateConfig(hotelId, req.body as any) } }
