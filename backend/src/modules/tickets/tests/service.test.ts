// tickets/tests/service.test.ts — Tests del servicio
...
function makeRepo(overrides: Partial<RepositoryAdapter<TicketsDTO>> = {}): RepositoryAdapter<TicketsDTO> {
  return {
    ...
    findById: async () => null,
    findOne: async () => null,
    create: async (data) => ({ id: 'test-id', ...data } as TicketsDTO),
...
describe('TicketsService', () => {
  describe('getById', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new TicketsService(makeRepo(), log, silentCache)
      await expect(service.getById('no-existe')).rejects.toThrow('Tickets no encontrado')
    })

    it('retorna el item si existe', async () => {
      const item = { id: '1' } as TicketsDTO
      const service = new TicketsService(makeRepo({ findById: async () => item }), log, silentCache)
  ...
  describe('delete', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new TicketsService(makeRepo({ delete: async () => false }), log, silentCache)
      await expect(service.delete('no-existe')).rejects.toThrow('Tickets no encontrado')
    })
  })
})
