// anuncios/tests/service.test.ts — Tests del servicio
...
function makeRepo(overrides: Partial<RepositoryAdapter<AnunciosDTO>> = {}): RepositoryAdapter<AnunciosDTO> {
  return {
    ...
    findById: async () => null,
    findOne: async () => null,
    create: async (data) => ({ id: 'test-id', ...data } as AnunciosDTO),
...
describe('AnunciosService', () => {
  describe('getById', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new AnunciosService(makeRepo(), log, silentCache)
      await expect(service.getById('no-existe')).rejects.toThrow('Anuncios no encontrado')
    })

    it('retorna el item si existe', async () => {
      const item = { id: '1' } as AnunciosDTO
      const service = new AnunciosService(makeRepo({ findById: async () => item }), log, silentCache)
  ...
  describe('delete', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new AnunciosService(makeRepo({ delete: async () => false }), log, silentCache)
      await expect(service.delete('no-existe')).rejects.toThrow('Anuncios no encontrado')
    })
  })
})
