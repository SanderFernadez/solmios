// opiniones/service.ts — Facade pública del módulo
...
//
// IMPORTANTE: depende de RepositoryAdapter<OpinionesDTO>, no del ORM directamente.
...
export class OpinionesService {
  ...
  async list(query?: OpinionesQuery): Promise<OpinionesPaginated> {
    this.logger.info('Listando opiniones', { query })
  ...
  async getById(id: string): Promise<OpinionesDTO> {
    this.logger.info('Obteniendo opiniones', { id })
    const item = await this.repo.findById(id)
    if (!item) throw new NotFoundError('Opiniones no encontrado')
  ...
  async create(dto: CreateOpinionesDTO): Promise<OpinionesDTO> {
    this.logger.info('Creando opiniones')
    const item = await this.repo.create(dto as Omit<OpinionesDTO, 'id'>)
    await this.sockets.onOpinionesCreated?.(item)
    await this.cache.delete('opiniones:list')
    return item
  }

  async update(id: string, dto: UpdateOpinionesDTO): Promise<OpinionesDTO> {
    this.logger.info('Actualizando opiniones', { id })
    const item = await this.repo.update(id, dto as Partial<Omit<OpinionesDTO, 'id'>>)
    if (!item) throw new NotFoundError('Opiniones no encontrado')
  ...
  async delete(id: string): Promise<void> {
    this.logger.info('Eliminando opiniones', { id })
    const deleted = await this.repo.delete(id)
    if (!deleted) throw new NotFoundError('Opiniones no encontrado')
