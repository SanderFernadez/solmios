// opiniones/tests/service.test.ts — Tests del servicio
...
function makeRepo(overrides: Partial<RepositoryAdapter<OpinionesDTO>> = {}): RepositoryAdapter<OpinionesDTO> {
  return {
    ...
    findById: async () => null,
    findOne: async () => null,
    create: async (data) => ({ id: 'test-id', ...data } as OpinionesDTO),
...
describe('OpinionesService', () => {
  describe('getById', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new OpinionesService(makeRepo(), log, silentCache)
      await expect(service.getById('no-existe')).rejects.toThrow('Opiniones no encontrado')
    })

    it('retorna el item si existe', async () => {
      const item = { id: '1' } as OpinionesDTO
      const service = new OpinionesService(makeRepo({ findById: async () => item }), log, silentCache)
  ...
  describe('delete', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new OpinionesService(makeRepo({ delete: async () => false }), log, silentCache)
      await expect(service.delete('no-existe')).rejects.toThrow('Opiniones no encontrado')
    })
  })
})
