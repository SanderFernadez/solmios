...
export class CrmController {
  ...
  async awardPoints(req: HttpRequest) {
    ...
    return { status: 201, body: await this.service.awardPoints(b.guestId, (req as any).user?.hotelId, b.points, b.description, b.reservationId) }
  }
  async redeemPoints(req: HttpRequest) {
    const b = validateSchema(RedeemPointsSchema, req.body) as any
    return { status: 200, body: await this.service.redeemPoints(b.guestId, (req as any).user?.hotelId, b.points, b.description) }
  }
  ...
  async getGuestsInSegment(req: HttpRequest) { return { status: 200, body: await this.service.getGuestsInSegment((req as any).user?.hotelId, req.params.id) } }
