// apikeys/tests/service.test.ts — Tests del servicio
...
function makeRepo(overrides: Partial<RepositoryAdapter<ApikeysDTO>> = {}): RepositoryAdapter<ApikeysDTO> {
  return {
    ...
    findById: async () => null,
    findOne: async () => null,
    create: async (data) => ({ id: 'test-id', ...data } as ApikeysDTO),
...
describe('ApikeysService', () => {
  describe('getById', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new ApikeysService(makeRepo(), log, silentCache)
      await expect(service.getById('no-existe')).rejects.toThrow('Apikeys no encontrado')
    })

    it('retorna el item si existe', async () => {
      const item = { id: '1' } as ApikeysDTO
      const service = new ApikeysService(makeRepo({ findById: async () => item }), log, silentCache)
  ...
  describe('delete', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new ApikeysService(makeRepo({ delete: async () => false }), log, silentCache)
      await expect(service.delete('no-existe')).rejects.toThrow('Apikeys no encontrado')
    })
  })
})
