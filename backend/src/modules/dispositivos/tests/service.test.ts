...
function makeRepo(overrides: Partial<RepositoryAdapter<DispositivosDTO>> = {}): RepositoryAdapter<DispositivosDTO> {
  return {
    ...
    findById: async () => null,
    findOne: async () => null,
    create: async (data) => ({ id: 'test-id', ...data } as DispositivosDTO),
...
describe('DispositivosService', () => {
  describe('getById', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new DispositivosService(makeRepo(), log, silentCache)
      await expect(service.getById('no-existe')).rejects.toThrow('Dispositivos no encontrado')
    })

    it('retorna el item si existe', async () => {
      const item = { id: '1' } as DispositivosDTO
      const service = new DispositivosService(makeRepo({ findById: async () => item }), log, silentCache)
  ...
  describe('delete', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new DispositivosService(makeRepo({ delete: async () => false }), log, silentCache)
      await expect(service.delete('no-existe')).rejects.toThrow('Dispositivos no encontrado')
    })
  })
})
