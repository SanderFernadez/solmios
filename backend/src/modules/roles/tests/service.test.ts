// roles/tests/service.test.ts — Tests del servicio
...
function makeRepo(overrides: Partial<RepositoryAdapter<RolesDTO>> = {}): RepositoryAdapter<RolesDTO> {
  return {
    ...
    findById: async () => null,
    findOne: async () => null,
    create: async (data) => ({ id: 'test-id', ...data } as RolesDTO),
...
describe('RolesService', () => {
  describe('getById', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new RolesService(makeRepo(), log, silentCache)
      await expect(service.getById('no-existe')).rejects.toThrow('Roles no encontrado')
    })

    it('retorna el item si existe', async () => {
      const item = { id: '1' } as RolesDTO
      const service = new RolesService(makeRepo({ findById: async () => item }), log, silentCache)
  ...
  describe('delete', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new RolesService(makeRepo({ delete: async () => false }), log, silentCache)
      await expect(service.delete('no-existe')).rejects.toThrow('Roles no encontrado')
    })
  })
})
