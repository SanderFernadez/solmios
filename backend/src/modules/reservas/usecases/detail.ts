...
export async function getExtendedDetail(repo: any, guestRepo: any, roomRepo: any, orm: any, id: string, currentUser: any): Promise<any> {
  ...
  const [guest, room, companions, lockCodes, payments, addons] = await Promise.all([
    ...
    orm?.findMany('Companions', { reservationId: r.id }) as Promise<any[]> || Promise.resolve([]),
    orm?.findMany('LockCodes', { reservationId: r.id }) as Promise<any[]> || Promise.resolve([]),
    orm?.findMany('PaymentRequests', { reservationId: r.id }) as Promise<any[]> || Promise.resolve([]),
    orm?.findMany('ReservationAddons', { reservationId: r.id }) as Promise<any[]> || Promise.resolve([]),
  ])
...
export async function getAuditTrail(repo: any, orm: any, id: string, currentUser: any): Promise<any[]> {
  const r = await repo.findById(id) as any
  if (!r) throw new NotFoundError('Reserva no encontrada')
  ...
  const logs = (await orm?.findMany('Auditlog', { entity: 'Reservations', entityId: id })) as any[] || []
