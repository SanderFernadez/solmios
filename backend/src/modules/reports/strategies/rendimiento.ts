...
export class RendimientoStrategy implements ReportStrategy {
  ...
  execute(ctx: ReportContext) {
    ...
    for (const r of res) { const rm = roomById.get(r.roomId); const t = rm?.type || 'unknown'; const n = nightsBetween(r.checkIn, r.checkOut); if (!adrByType[t]) adrByType[t] = { nights: 0, revenue: 0, adr: 0 }; adrByType[t].nights += n; adrByType[t].revenue += r.totalAmount || 0 }
