...
export class ReportsService {
  ...
  async getReports(req: any): Promise<any> {
    ...
    const [res, rooms, guests] = await Promise.all([
      this.orm.findMany('Reservations', { hotelId: id }) as Promise<any[]>,
      this.orm.findMany('Rooms', { hotelId: id }) as Promise<any[]>,
      this.orm.findMany('Guests', { hotelId: id }) as Promise<any[]>,
    ])
  ...
  async getAdvancedReport(req: any): Promise<any> {
    ...
    const [reservations, rooms, guests, expenses, folioCharges, blocks, hotel] = await Promise.all([
      ...
      this.orm.findMany('Rooms', { hotelId: id }) as Promise<any[]>,
      this.orm.findMany('Guests', { hotelId: id }) as Promise<any[]>,
      this.orm.findMany('Expenses', { hotelId: id }) as Promise<any[]>,
      this.orm.findMany('FolioCharges', { hotelId: id }) as Promise<any[]>,
      this.orm.findMany('RoomBlocks', { hotelId: id }) as Promise<any[]>,
      (await this.orm.findMany('Hotels', { id }))[0] as any,
    ])
  ...
  async getNightAudit(req: any): Promise<any> {
    ...
    const rooms = await this.orm.findMany('Rooms', { hotelId: id }) as any[]
