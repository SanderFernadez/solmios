...
describe('HuespedesService', () => {
  describe('getById', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new HuespedesService(makeRepo(), log, silentCache)
      await expect(service.getById('no-existe')).rejects.toThrow('Huespedes no encontrado')
    })

    it('retorna el item si existe', async () => {
      const item = { id: '1' } as HuespedesDTO
      const service = new HuespedesService(makeRepo({ findById: async () => item }), log, silentCache)
  ...
  describe('delete', () => {
    it('lanza NotFound si el item no existe', async () => {
      const service = new HuespedesService(makeRepo({ delete: async () => false }), log, silentCache)
      await expect(service.delete('no-existe')).rejects.toThrow('Huespedes no encontrado')
    })
  })
})
