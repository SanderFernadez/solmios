...
describe('rateLimit(key, opts?) — F0 extensión retrocompatible', () => {
  it('sin opts: 20 llamadas permiten, la 21ª bloquea con retryAfter (reproduce los 6 call-sites reales)', () => {
  ...
  it('con opts={maxAttempts:120, windowMs:300000}: una key distinta permite 120 intentos sin afectar keys sin opts', () => {
    const publicKey = `public-menu:${crypto.randomUUID()}`
    const otherKey = `login:${crypto.randomUUID()}`

    for (let i = 0; i < 120; i++) {
      const { allowed } = rateLimit(publicKey, { maxAttempts: 120, windowMs: 300000 })
      expect(allowed).toBe(true)
    }
    const blocked = rateLimit(publicKey, { maxAttempts: 120, windowMs: 300000 })
    expect(blocked.allowed).toBe(false)
    expect(blocked.retryAfter).toBeGreaterThan(0)

    // La key sin opts (bucket independiente) sigue permitiendo sus 20 intentos normales
    for (let i = 0; i < 20; i++) {
      const { allowed } = rateLimit(otherKey)
      expect(allowed).toBe(true)
    }
    const otherBlocked = rateLimit(otherKey)
  ...
  it('ventana expira: tras windowMs, el contador resetea (mismo criterio ya implementado)', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const { allowed } = rateLimit(key, { maxAttempts: 3, windowMs: shortWindow })
      expect(allowed).toBe(true)
    }
    expect(rateLimit(key, { maxAttempts: 3, windowMs: shortWindow }).allowed).toBe(false)

    await new Promise((resolve) => setTimeout(resolve, shortWindow + 20))

    // Ventana expirada: el contador resetea, permite de nuevo
    expect(rateLimit(key, { maxAttempts: 3, windowMs: shortWindow }).allowed).toBe(true)
  })
})
